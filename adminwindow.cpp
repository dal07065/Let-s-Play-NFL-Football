...
void adminWindow::on_comboBox_SelectSouvenir_activated(const QString &arg1)
{
    ...
    QSqlQuery query;
    query.prepare(QString("SELECT SouvenirName, Price, TeamId FROM teams_souvenir"));
    ...
    else
    {
        while(query.next())
        {
            ...
            && query.value(2) == ui->comboBox_Team_Selection->currentIndex() + 1)
            {
...
//    query.bindValue(":_id", stadiumID);
    ...
    query.prepare(QString("UPDATE stadiums SET StadiumName = :StadiumName WHERE _id = :_id"));
    ...
    query.prepare(QString("UPDATE stadiums SET SeatingCapacity = :SeatingCapacity WHERE _id = :_id"));
    ...
    query.prepare(QString("UPDATE stadiums SET DateOpened = :DateOpened WHERE _id = :_id"));
